...
;>>>>> begin main program code <<<<<
			...
			BL		Init_PIT_IRQ
			CPSIE	I
Loop		;5 Output prompt
			LDR		R0, =AccessStr
			MOVS	R1, #(AccessPast - AccessStr)
			BL		PutStringSB
			BL		PutCRLF
			MOVS	R0, #BEGIN_CH
			BL		PutChar
...
CompLoop	LDRB	R2, [R0, R4]
			...
			ADDS	R4, R4, #1
			B		CompLoop
Failure		LDR		R0, =DeniedStr
			MOVS	R1, #(DeniedPast - DeniedStr)
			BL		PutStringSB
			BL		PutCRLF
			B		Loop
Success		LDR		R0, =Count							;Check timer
			LDR		R0, [R0, #0]
			LDR		R1, =Five
			LDR		R1, [R1, #0]
			CMP		R0, R1
			BHI		Failure								;If too late, fail
			;11 Output Success
			LDR		R0, =GrantedStr
			MOVS	R1, #(GrantedPast - GrantedStr)
			BL		PutStringSB
			BL		PutCRLF
			;12 Output Complete
			LDR		R0, =SuccessStr
			MOVS	R1, #(SuccessPast - SuccessStr)
			BL		PutStringSB
			BL		PutCRLF
...
AccessStr	DCB		"Enter the access code.", 0
AccessPast
FactorStr	DCB		" x 0.01 s", 0
FactorPast
GrantedStr	DCB		"Access granted", 0
GrantedPast
DeniedStr	DCB		"Access denied", 0
DeniedPast
SuccessStr	DCB		"Mission completed!", 0
SuccessPast
CodeStr		DCB		"25015110", 0
Five		DCD		500
;>>>>>   end constants here <<<<<
            ALIGN
;****************************************************************
...
TxQRecord   	SPACE   Q_REC_SZ  ; Transmit queue management record
				KEEP    TxQRecord
RxQBuffer   	SPACE   Q_BUF_SZ  ; Receive queue buffer
				KEEP    RxQBuffer
				ALIGN
