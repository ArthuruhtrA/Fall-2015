...
;>>>>> begin main program code <<<<<
			BL		Init_UART0_IRQ		;Initialize UART0 IRQ
			CPSIE	I
			LDR		R0, =QBuffer		;Initialize program queue
			LDR		R1, =QRecord
			MOVS	R2, #Q_BUF_SZ
			BL		InitQueue
Prompt		LDR		R0, =PromptStr		;Prompt
			MOVS	R1, #(PromptPast - PromptStr)
			BL		PutStringSB
			BL		GetChar				;Get input
			
			BL		PutChar				;Output char
			BL		PutCRLF				;Output CRLF
			
			;Convert capitals to lowercase
			;Check if character is >= A and <= Z
			CMP		R0, #'A'
			BLT		CapsDone
			CMP		R0, #'Z'
			BGT		CapsDone
			;If so, subtract A, then add a
			SUBS	R0, R0, #'A'
			ADDS	R0, R0, #'a'
			
CapsDone	CMP		R0, #'d'			;Check for valid input,
			BEQ		d					;Then branch to proper code
			CMP		R0, #'e'
			BEQ		e
			CMP		R0, #'h'
			BEQ		h
			...
			CMP		R0, #'s'
			BEQ		s
			B		Prompt				;Otherwise loop to prompt
d			LDR		R1, =QRecord		;D equeue
			BL		Dequeue
			BCC		dcont
			LDR		R0, =FailureStr
			MOVS	R1, #(FailPast - FailureStr)
			BL		PutStringSB
			B		step8
dcont		BL		PutChar				;Write to terminal
			...
			BL		PutChar
			B		step8
e			LDR		R0, =EnqueueStr		;E nqueue (prompt)
			MOVS	R1, #(EnqPast - EnqueueStr)
			BL		PutStringSB
			BL		GetChar				;Get input
			BL		PutChar
			BL		PutCRLF				;Output CRLF
			LDR		R1, =QRecord
			BL		Enqueue
			MOVS	R1, #0x9			;Length of both FailStr and StatusStr
			BCC		e2
			LDR		R0, =FailureStr
			BL		PutStringSB
			B		step8
e2			LDR		R0, =SuccessStr
			BL		PutStringSB
			B		step8
h			LDR		R0, =HelpStr		;H elp (list commands)
			MOVS	R1, #(HelpPast - HelpStr)
			BL		PutStringSB
			BL		PutCRLF
			B		Prompt
p			MOVS	R0, #Q_BEGIN_CH		;P rint queue
			BL		PutChar
			;Loop through queue, print each char
			LDR		R1, =QRecord
			LDRB	R2, [R1, #NUM_ENQD]
			LDR		R3, [R1, #BUF_PAST]
			LDR		R4, [R1, #OUT_PTR]
Loop		CMP		R2, #0
			...
			LDR		R4, [R1, #BUF_STRT]
			B		Loop
Cont		MOVS	R0, #Q_END_CH
			BL		PutChar
			BL		PutCRLF
			B		Prompt
s			;S tatus (InPointer, OutPointer, NumberEnqueued)
			LDR		R0, =StatusStr
			MOVS	R1, #(StatusPast - StatusStr)
			BL		PutStringSB
step8		;InStr 8, OutStr A, NumStr 8
			LDR		R2, =QRecord
			;In
			LDR		R0, =InStr
			MOVS	R1, #(InPast - InStr)
			BL		PutStringSB
			LDR		R0, [R2, #IN_PTR]
			BL		PutNumHex
			;Out
			LDR		R0, =OutStr
			MOVS	R1, #(OutPast - OutStr)
			BL		PutStringSB
			LDR		R0, [R2, #OUT_PTR]
			BL		PutNumHex
			;Num
			LDR		R0, =NumStr
			MOVS	R1, #(NumPast - NumStr)
			BL		PutStringSB
			LDRB	R0, [R2, #NUM_ENQD]
			BL		PutNumUB
			BL		PutCRLF
...
PromptStr	DCB		"Type a queue command (d, e, h, p, s):", 0
PromptPast
EnqueueStr	DCB		"Character to enqueue:", 0
EnqPast
FailureStr	DCB		"Failure:", 0
FailPast
SuccessStr	DCB		"Success:", 0
SuccPast
HelpStr		DCB		"d (dequeue), e (enqueue), h (help), p (print), s (status)", 0
HelpPast
StatusStr	DCB		"Status:", 0
StatusPast
InStr		DCB		" In = 0x", 0
InPast
OutStr		DCB		" Out = 0x", 0
