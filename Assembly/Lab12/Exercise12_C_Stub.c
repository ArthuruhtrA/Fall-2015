/*********************************************************************/
/* Provides control via CLI for servo operation                      */
/* Using DAC, ADC, and PWM                                           */
...
int main (void) {
  ...
  /* Perform all device initialization here */
	...
	do {
		...
		/* Wait for calibration to copmlete */
		while (ADC0->SC3 & ADC_SC3_CAL_MASK);
		/* Check for calibration failure */
	} while (ADC0->SC3 & ADC_SC3_CALF_MASK);
	ADC0->PG = (((Int16) ADC0->CLP0 + (Int16) ADC0->CLP1 + \
							(Int16) ADC0->CLP2 + (Int16) ADC0->CLP3 + \
							(Int16) ADC0->CLP4 + (Int16) ADC0->CLPS) \
							>> 1) | (Int16) 0x8000;
	ADC0->MG = (((Int16) ADC0->CLM0 + (Int16) ADC0->CLM1 + \
							(Int16) ADC0->CLM2 + (Int16) ADC0->CLM3 + \
							(Int16) ADC0->CLM4 + (Int16) ADC0->CLMS) \
							>> 1) | (Int16) 0x8000;
  ...
  for (;;) { /* do forever */
		//5 Request input
		PutStringSB("Type a number from 1 to 5: ", 28);
		//6 Get input
		input = GetChar();
		//7 Keep trying until valid input
		while (input < 49 || input > 53){
			input = GetChar();
		}
		PutChar(input);
		PutCRLF();
		//8 Check table for value
		output = DAC0_table[input - 49];
		//9 Calculate original value
		DAC0->DAT[0].DATL = (UInt8) (output & 0xFF);
		DAC0->DAT[0].DATH = (UInt8) (output >> 8);
		//10 Output original value
		PutStringSB("Original digital value: 0x", 27);
		PutNumHex(output);
		PutCRLF();
		//11 Calculate new value
		ADC0->SC1[0] = ADC0_SC1_SGL_DAC0;
		while (!(ADC0->SC1[0] & ADC_SC1_COCO_MASK));
		output = ADC0->R[0];
		//12 Output new value
		PutStringSB("New digital value: 0x", 22);
		PutNumHex(output);
		PutCRLF();
		//13 Scale to duty count value
		input = (output * 5) / 1024;
		output = PWM_duty_table[input];
		//14 Set position
		TPM0->CONTROLS[4].CnV = output;
		//15 Output position
		PutStringSB("Servo position: ", 17);
		PutNumUB(input + 1);
		PutCRLF();
		//16 Loop to 6
  } /* do forever */
